...
const PaymentFrom = () => {
    ...
    useEffect(() => {
        reset({ cus_name: user.displayName, cus_email: user.email, date: new Date().toLocaleDateString() })
    }, [reset, user.displayName, user.email])


    const onSubmit = async (data) => {
        const paymentData = await { ...data, total_amount: grandtotal, cartBooks: [cartBooks], status: "pending" }
        await axios.post(`${api}/init`, paymentData)
            .then(res => {
                console.log(res)
            }).catch((error) => {
                console.log(error)
    ...
    return (
        ...
        <Box>
            <form onSubmit={handleSubmit(onSubmit)}>
                ...
                <Button type="submit" sx={{ ...ButtonStyle, width: "50%" }} >Submit</Button>
            </form>
        </Box>
